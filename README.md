...
- 👥 **Social networks → records** — posts from X as normalized rows (text, author, date, likes, media) by intercepting the network's own internal API and matching posts **by shape**, so it survives their weekly reshuffles. Needs a logged-in session; see [docs/SOCIAL.md](docs/SOCIAL.md).
