...
- 👥 **Social networks → records** — posts from **X, LinkedIn and Facebook** as normalized rows (text, author, date, likes, media) by intercepting the network's own internal API and matching posts **by shape**, so it survives their weekly reshuffles. Needs a logged-in session; see [docs/SOCIAL.md](docs/SOCIAL.md).
- 🖱️ **Browser actions** — don't just read the page, *operate* it: dismiss the cookie banner, log in, press "load more", exhaust an infinite feed, grab a screenshot or PDF — declaratively, no Playwright code. Named **sessions** keep a login alive across jobs. See [docs/ACTIONS.md](docs/ACTIONS.md).
